...
const asignarValorDolar = async () => {
  const precioDolar = document.getElementById("dolar");

  // Obtener el valor actual del dólar
  const valorDolar = await obtenerValorDolar();

  if (valorDolar !== null) {
    // Asignar el valor del dólar a la caja de texto "precio dólar"
    precioDolar.value = valorDolar;
  } else {
    // Manejar el caso en el que no se puede obtener el valor del dólar
    console.warn("No se pudo obtener el valor del dólar. Asignando un valor predeterminado.");
    precioDolar.value = "Valor predeterminado";
  }
};
asignarValorDolar()
